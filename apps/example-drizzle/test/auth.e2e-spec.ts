...
const SQLITE_PATH = resolve(__dirname, "e2e-drizzle.sqlite");
process.env["KEYCLOAK_SERVER_URL"] = "http://localhost:8080";
process.env["SQLITE_PATH"] = SQLITE_PATH;
process.env["ACCESS_TOKEN_SECRET"] = "e2e-drizzle-access-secret-never-in-prod";
process.env["REFRESH_TOKEN_SECRET"] = "e2e-drizzle-refresh-secret-never-in-prod";
...
describe("Drizzle store E2E", () => {
  ...
  const user = {
    email: `dz-${uid()}@test.com`,
    username: `dz${uid()}`,
    password: "StrongPass1!",
  };

  it("registers a user (persisted via Drizzle)", async () => {
